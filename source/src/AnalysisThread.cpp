...
void AnalysisThread::HitMap()
{
  ...
  TList *m_histoLogMap = new TList();
  m_histoEnergyMap->SetName("HitMap_Log List");
  //Loop over rootfile
  for(int n = 0; n < tree->GetEntries(); n++)
    {
      tree->GetEntry(n);
  ...
  for(int ilayer = 0; ilayer < nLayer; ilayer++)
    {
      ...
      m_histoLogMap->Add(pHisto[ilayer]);
  ...
  pArchive->mkdir("HitMap_Log");
  pArchive->WriteElement(m_histoLogMap);
  pArchive->close();
  mutex.unlock();
